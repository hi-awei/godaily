<?php
error_reporting(0);
// 强制 www 301 重定向，统一规范URL
if (!isset($_SERVER['HTTP_HOST']) || strpos($_SERVER['HTTP_HOST'], 'www.') !== 0) {
    $proto = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    header('Location: ' . $proto . '://www.993899.com' . $_SERVER['REQUEST_URI'], true, 301);
    exit;
}
...
$pageTitle = '首页';
$pageDesc = '每日更新，收录精选AI工具，为你发现最值得使用的AI产品。涵盖AI对话、AI绘画、AI编程、大模型等分类。';
...
try {
    ...
    $newTools = $db->query("SELECT * FROM tools WHERE status=1 AND category != 'ai-research' ORDER BY created_at DESC LIMIT 10")->fetchAll();
    $categoryStats = $db->query("SELECT category, COUNT(*) as cnt FROM tools WHERE status=1 GROUP BY category ORDER BY cnt DESC LIMIT 10")->fetchAll();
    $totalTools = $db->query("SELECT COUNT(*) FROM tools WHERE status=1")->fetchColumn();
} catch (Exception $e) {
    $featuredTools = $newTools = $categoryStats = [];
    $totalTools = 0;
}
...
<section class="hero">
    <div class="container">
        ...
        <p class="hero-sub">AI导航 每日更新，收录 <?= number_format($totalTools) ?>+ 款精选AI工具</p>
