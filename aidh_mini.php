<?php
// aidh_mini.php - 设置 application/octet-stream 绕过nginx gzip
error_reporting(0);
ini_set('display_errors', 0);
ini_set('zlib.output_compression', '0');
header('Content-Type: application/octet-stream; charset=ASCII');
header('Cache-Control: no-store');
define('SECRET_KEY', 'godaily2026');
if (!isset($_GET['k']) || $_GET['k'] !== SECRET_KEY) { http_response_code(403); echo "403"; exit; }
if (!isset($_GET['do'])) { echo "READY"; exit; }
$db = @new mysqli('localhost', 'web01_com', '3FT7Ppatfp19XbAh', 'web01_com');
if ($db->connect_error) { echo "E:DB"; exit; }
...
if (!$json || empty($json['collected'])) { echo "E:JSON"; exit; }
