<?php
// aidh_buffer.php - 强制禁用nginx gzip：用输出缓冲+chunked trick
// nginx的gzip对chunked响应会尝试压缩，但若我们flush到chunk再输出，gzip可能来不及
error_reporting(0);
...
if (!isset($_GET['k']) || $_GET['k'] !== SECRET_KEY) { http_response_code(403); ob_end_flush(); exit; }
if (!isset($_GET['do'])) { echo "OK"; ob_end_flush(); exit; }

$db = @new mysqli('localhost', 'web01_com', '3FT7Ppatfp19XbAh', 'web01_com');
if ($db->connect_error) { echo "E:DB"; ob_end_flush(); exit; }
...
if (!$json || empty($json['collected'])) { echo "E:JSON"; ob_end_flush(); exit; }
