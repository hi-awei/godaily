...
if (!isset($_GET['key']) || $_GET['key'] !== SECRET_KEY) { header('HTTP/1.1 403'); die('403'); }
if (!isset($_GET['do'])) {
    echo "Ready. Add &do=1\n"; exit;
}
...
if ($db->connect_error) { fwrite($out, "DB error: " . $db->connect_error . "\n"); fclose($out); echo "OK"; exit; }
...
if (!$json || empty($json['collected'])) { fwrite($out, "No data\n"); fclose($out); echo "OK"; exit; }

$total = count($json['collected']);
fwrite($out, "Starting $total tools...\n");
...
foreach ($json['collected'] as $t) {
    ...
    if ($ok % 50 === 0 && $ok > 0) { fwrite($out, "Progress: $ok OK, $dup Dup, $err Err\n"); }
}

fwrite($out, "DONE OK:$ok Dup:$dup Err:$err\n");
fclose($out);
echo "OK"; // Short response - no gzip issue
